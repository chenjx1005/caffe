...
class CsrFunctionsGenTest : public ::testing::Test {
 protected:
  ...
  virtual void SetUp(int m, int n, int k, int nzz, int ptr_size) {
    ...
    indices_.reset(new SyncedMemory(nzz * sizeof(Dtype)));
    ptr_.reset(new SyncedMemory(ptr_size * sizeof(Dtype)));
  ...
  virtual void run(bool isCpu, int times = 1) {
    if (isCpu) {
      ...
      for (int t = 0; t < times; t++) {
        caffe_cpu_csrgemm(TransA, TransB, M, N, K, alpha, cpu_A(),
...
#ifndef CPU_ONLY
      Dtype* agpu = gpu_A();
      Dtype* indicesgpu = gpu_indices();
      Dtype* ptrgpu = gpu_ptr();
      Dtype* bgpu = gpu_B();
      Dtype* cgpu = gpu_C();
      Timer timer;
      timer.Start();
      for (int t = 0; t < times; t++) {
        caffe_gpu_csrgemm(TransA, TransB, M, N, K, alpha, agpu,
  ...
  void setA(Dtype A_data[], Dtype A_indices[], Dtype A_ptr[]) {
    Dtype* am = cpu_A();
    Dtype* aindices = cpu_indices();
    Dtype* aptr = cpu_ptr();
  ...
  Dtype* cpu_indices() {
    CHECK(indices_);
    return reinterpret_cast<Dtype*>(indices_->mutable_cpu_data());
  }
  Dtype* gpu_indices() {
    CHECK(indices_);
    return reinterpret_cast<Dtype*>(indices_->mutable_gpu_data());
  }

  Dtype* cpu_ptr() {
    CHECK(ptr_);
    return reinterpret_cast<Dtype*>(ptr_->mutable_cpu_data());
  }
  Dtype* gpu_ptr() {
    CHECK(ptr_);
    return reinterpret_cast<Dtype*>(ptr_->mutable_gpu_data());
  }

  void random_csr(int M, int N, int nzz_per_row, Dtype* A, Dtype* indices,
                  Dtype* ptr) {
    srand(0);
    ptr[0] = 0;
  ...
  void test_speed_forward(int batch_size, int features, int nzz_per_row,
    ...
    Dtype* indices = new Dtype[batch_size * nzz_per_row];
    Dtype* ptr = new Dtype[batch_size + 1];
